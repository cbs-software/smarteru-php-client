...
class User {
    ...
    public function toXML(
        ...
        if ($methodName === 'updateUser') {
            ...
            if (!empty($this->getOldEmail())) {
                $identifier->addChild('Email', $this->getOldEmail());
            } else if (!empty($this->getOldEmployeeId())) {
                $identifier->addChild('EmployeeID', $this->getOldEmployeeId());
            } else {
                ...
                if (!empty($this->getEmail())) {
                    $identifier->addChild('Email', $this->getEmail());
                } else {
                    $identifier->addChild('EmployeeID', $this->getEmployeeId());
                }
            }
        }
        ...
        if ($methodName === 'createUser') {
            ...
            $info->addChild(
                ...
                $this->getLearnerNotifications() ? '1' : '0'
            );
            $info->addChild(
                'SupervisorNotifications',
                $this->getSupervisorNotifications() ? '1' : '0'
            );
            ...
            if (!empty($this->getOldEmail())) {
                $info->addChild('Email', $this->getEmail());
                $this->setOldEmail(null);
            }
            if (!empty($this->getOldEmployeeId())) {
                $info->addChild('EmployeeID', $this->getEmployeeId());
                $this->setOldEmployeeId(null);
            }
            ...
            if (!empty($this->getSendEmailTo())) {
                $info->addChild('SendEmailTo', $this->getSendEmailTo());
            }
            if (!empty($this->getAlternateEmail())) {
                $info->addChild('AlternateEmail', $this->getAlternateEmail());
            }
        ...
        foreach ($this->getGroups() as $group) {
            ...
            if ($methodName === 'updateUser') {
                $groupTag->addChild('GroupAction', $group->getAction());
            }
